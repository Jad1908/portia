...
from portia.core.io import connect, source_table, write_table
...
def unreachable_steps(doc: dict) -> list[str]:
    """Steps nothing downstream reads — in spec order, and a **fact, not a verdict**.

    A spec produces one table and that table is its **last step**: `compile_spec`
    ends the file with ``SELECT * FROM <last step>``. So a step is part of the
    table exactly when the last step transitively reads it, and any step outside
    that set is a named CTE nothing selects from — still compiled into the
    ``.sql``, and still **executed on every run**, because `run_spec` runs each
    step in order to measure it.

    They arrive honestly. Steps are append-only (`handlers.record_step` runs a
    step and appends it), so a superseded attempt stays on the record: the demo
    project has four consecutive tries at one aggregation, and nothing anywhere
    could say so. That is the right default — the decision record is a record,
    and silently dropping a step the agent recorded would be the artifact editing
    itself. What was missing is that the cost is invisible.

    **This names them and stops.** Whether an unread step is a mistake to delete
    or a working alternative kept on purpose is judgment, and judgment is the
    agent's and the human's (`CLAUDE.md` — facts vs judgment). Nothing here
    removes one, reorders one, or calls one wrong.
    """
    steps = [s for s in (doc.get("steps") or []) if s.get("id")]
    if not steps:
        return []
    inputs = {s["id"]: step_inputs(s) for s in steps}

    live: set[str] = set()
    stack = [steps[-1]["id"]]
    while stack:
        current = stack.pop()
        if current in live or current not in inputs:
            continue
        live.add(current)
        stack += inputs[current]
    return [s["id"] for s in steps if s["id"] not in live]
...
def spec_path(name: str, *, layer: str | None = None, root: str | Path = ".") -> Path:
    """Where the spec that produces ``name`` belongs — mirroring `pipeline.model_path`.
    ...
    **The engine places the file, not the caller** *(2026-08-16)*. The decision
    record and the build output should sit in the same shape, and until now only
    the build output did: `models/` grew `staging/`, `intermediate/` and `mart/`
    subdirectories while every spec stayed flat in `specs/`. Deriving the path
    from the `layer` the spec already declares is what stops the two disagreeing —
    a spec that says `staging` cannot end up anywhere else, because nothing gets
    to pass a path.
    """
    base = Path(root) / SPECS_DIR
    return (base / layer if layer else base) / f"{name}.yaml"
...
def validate_target(target: Any) -> None:
    """Where a spec's table is created on a warehouse, ``DATABASE.SCHEMA``; none is fine.

    **A spec-level claim, chosen per table** (`docs/CONNECTOR.md` §2.7.1, the
    user's call): different tables go different places, so there is no project
    setting for it and nothing in the engine that defaults it. A spec on a
    warehouse without one is refused at build time, naming the spec.
    """
    if target is None:
        return
    parts = str(target).split(".") if isinstance(target, str) else []
    if len(parts) != 2 or not all(p.strip() for p in parts):
        raise ValueError(f"a target is DATABASE.SCHEMA, not {target!r}")
...
def upstream_of(
    name: str | Iterable[str], models: dict[str, Path], *, base_dir: str | Path = "."
) -> list[str]:
    ...
    **Several names are one scope, not several runs** *(2026-08-15)*. The app's
    canvas can be narrowed to more than one table, and Run there means *run what
    is on screen* — so the closure is taken over the whole selection at once and
    the result stays a single build order. Running them one at a time would
    rebuild a shared upstream once per branch, and the durable artifacts of the
    second build would be the ones that survived, which is a build order chosen
    by accident.
    """
    wanted_roots = [name] if isinstance(name, str) else list(name)
    unknown = [n for n in wanted_roots if n not in models]
    if unknown:
        known = ", ".join(sorted(models))
        raise ValueError(f"no spec produces {unknown[0]!r}. In this project: {known}")
...
def run_spec(
    ...
    **Nothing is materialized.** Sources and step outputs alike are relations, so
    ...
    ``on_step(done, total, step_id)`` is called **before** each step, with the
    number already finished — the same shape as `ui/engine.index`, so the two
    things in this app that report progress report it one way. It is how a
    surface can name the step being executed instead of saying only that
    something is happening, which on a spec whose steps differ by two orders of
    magnitude in cost is the difference between a wait and a hang. Nothing here
    times a step or decides what a caller does with the number; a `None` callback
    is the CLI, and costs a branch per step.

    **A nested model run is not reported.** `model_table` re-executes an upstream
    spec to resolve an input, and its steps are not steps of *this* spec — a
    surface counting them would show a total that grows as the run proceeds.
    """
    base = Path(base_dir)
    con = con or connect()
    # A source is a path relative to the project, or `{table: …}` naming a table
    # the connection can already see (`core/io.source_table`, `CONNECTOR.md` §2.5).
    tables: dict[str, Table] = {
        name: source_table(ref, con, base=base, name=name)
        for name, ref in (spec.get("sources") or {}).items()
    }

    results: list[StepResult] = []
    steps = spec.get("steps", [])
    for done, step in enumerate(steps):
        # Before the step, not after: an interrupt that landed in the gap between
        # two queries interrupted nothing, and without this the run would go on
        # into the step Stop was pressed to avoid (`core/cancel.py`).
        cancel.check()
        if on_step is not None:
            on_step(done, len(steps), str(step.get("id") or ""))
...
def write_report(
    ...
    *,
    ...
    ``name`` is the model's name, and it is in the filename for the same reason
    it is in a written table's (`write_outputs`): a build produces several tables
    and saving them writes a report each, so a timestamp alone would have them
    landing on top of one another inside the same second.
    """
