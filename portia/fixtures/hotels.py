...
- ``reservations`` (rooms already booked) knows only ``hotel_id``. It has no city.
...
in the fact table. An agent that only inspects ``reservations`` and ``city_events`` finds
no join at all.
...
- **Fan-out that silently double-counts revenue.** Amsterdam has *two* events on
  2026-06-12. Joining ``reservations`` to ``city_events`` on city+date multiplies those
...
def reservations() -> pd.DataFrame:
    """Rooms already booked. Knows the hotel, but nothing about geography."""
    return pd.DataFrame(
        {
