...
def _header(
    ...
    *,
    ...
    if spec_path:
        # Forward slashes: this file is committed, and a header that changed with the
        # machine that built it would be a diff on every build (`core/io.relative`).
        lines.append(f"-- Change {Path(spec_path).as_posix()} and regenerate; that file holds the")
