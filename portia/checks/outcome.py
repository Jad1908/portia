...
#: Every field this report comes back with. Named so that `agent.handlers` can
#: tell one specific mistake from a typo: a step's `expect` block asserts what
#: the **op** reported, and these are the words of the *other* report that comes
#: back from the same call. `n_rows` in particular is the one a reader meets
#: first — it is right there in the outcome block — and reaching for it in
#: `expect` is a confusion between two questions rather than a guess.
#: `tests/test_outcome.py` fails if a real report stops matching this.
...
def _contribution(
    ...
    *,
    ...
    # **Absent when nothing reached the output at all**, and absent rather than
    # empty: `[]` would say *nothing was dropped*, which is the opposite of what
    # happened. In that case `missing` is this input's whole non-key column list
    # restated, derivable from `n_columns: 0` plus a schema the reader already
    # has, and attached to a case this check has just declined to judge —
    # `contributed` is None and `_flags` raises nothing.
    #
    # It is also the case where the word is least true. Matching is by name, so
    # a step that renames every column (`SELECT id AS hotel_id …`) carries every
    # value through and lands here indistinguishable from `SELECT count(*)`.
    # Calling those 191 columns *dropped* is a claim the measurement cannot make.
    #
    # Measured on the AQN build run: 24,999 of `columns_dropped`'s 30,049
    # characters came from the seven inputs in this state. The 5,046 that remain
    # are the partial drops — `['TITLE', 'CATEGORY', 'RANK']` off a join that was
    # meant to carry event data — which is the report doing its job.
    if reached:
        report["columns_dropped"] = missing
    return report
...
def _table_grain(table: Table, grain: list[str]) -> dict:
    # Spelled as the output spells them: a warehouse folds an unquoted alias
    # to upper case, and the claim was written before the table existed
    # (`dialect.resolve_columns`). The report then names the real columns.
    keys = dialects.resolve_columns([str(c) for c in grain], table.columns)
    early = _grain(keys, [c for c in keys if c not in table.columns])
    if early:
        return early

    quoted = ", ".join(table.dialect.quote(c) for c in keys)
