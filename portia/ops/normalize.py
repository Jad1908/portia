...
#: Each written against the dialect at hand (`_expression`): the text and float
#: type names and the try-cast form are the three things engines spell
#: differently here, and nothing else in these is engine-specific.
_EXPRESSIONS = {
    "strip": "trim(CAST({col} AS {text}))",
    "lower": "lower(CAST({col} AS {text}))",
    "to_numeric": "{try_cast}",
    "to_string": "CAST({col} AS {text})",
}


def _expression(op: str, quoted: str, dialect: dialects.Dialect) -> str:
    return _EXPRESSIONS[op].format(
        col=quoted, text=dialect.text, try_cast=dialect.try_cast(quoted, dialect.double)
    )
...
def apply_normalize(table: Table, transforms: list[dict], *, name: str | None = None) -> OpResult:
    ...
    records = []
    for t in transforms:
        (col,), op = dialects.resolve_columns([t["column"]], out.columns), t["op"]
        ...
        expression = _expression(op, out.dialect.quote(col), out.dialect)
...
def _project(
    columns: list[str], column: str, expression: str, from_item: str, dialect: dialects.Dialect
) -> str:
    ...
    """
    q = dialect.quote
    select = ", ".join(f"{expression} AS {q(c)}" if c == column else q(c) for c in columns)
...
def _measure(table: Table, column: str, op: str, expression: str) -> dict:
    ...
    if op == "to_numeric":
        ...
        counts = table.row(
            {"n_converted": f"count({expression})", "n_failed": d.count_where(failed)}
        )
        record: dict = {
    ...
    text = f"CAST({q} AS {d.text})"
    changed = f"{text} IS NOT NULL AND {text} <> {expression}"
    return {"n_changed": int(table.scalar(d.count_where(changed)))}
