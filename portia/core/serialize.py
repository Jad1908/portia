...
def to_json(obj: Any) -> str:
    ...
    Indented, because the readers are human: `python -m portia.cli.profile --json`
    and `cli/history` print it at a terminal. The copy the *model* reads is
    `to_json_compact` — same evidence, and the indent is 36% of the payload.
    """
    return json.dumps(obj, indent=2, ensure_ascii=False)


def to_json_compact(obj: Any) -> str:
    """The same evidence, for a reader that pays by the character.

    Every tool result goes through here (`agent/tools.py`). Measured on the AQN
    build run's `profile_source` payload: 83,079 characters indented, 53,518
    compact, for identical content. The separators matter as much as the indent —
    ``json.dumps`` still pads with ``", "`` and ``": "`` when you only drop the
    indent, which is two characters back on every field of every record.

    No ``default=`` fallback on purpose, unlike `to_json_line`. A tool payload is
    jsonable by contract (`agent/handlers.py` returns plain dicts), so something
    unserializable reaching here is a bug worth raising rather than stringifying
    into the evidence.
    """
