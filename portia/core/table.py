...
def quote_ident(name: str) -> str:
    ...
    Anything holding a connection asks the connection's dialect instead
    (:attr:`Table.dialect`): the hatch's sandbox and a fixture frame are DuckDB
    whatever the project runs on, and this is theirs. `Dialect.quote` is the
    one implementation; this is its local name.
    """
    return dialects.DUCKDB.quote(name)
...
class Table:
    ...
    def dialect(self) -> dialects.Dialect:
        """How the engine this table is read on spells things (`dialect.of`)."""
        return dialects.of(self.con)
    ...
    def ref(self) -> str:
        ...
        cannot be used here: ``WITH "reservations" AS (SELECT * FROM "reservations")`` is a CTE
    ...
    def from_name(cls, name: str, con: Any) -> Table:
        ...
        return cls(name=name, query=f"SELECT * FROM {dialects.of(con).quote(name)}", con=con)
