...
#: Where a duration stops being worth a decimal, and where it stops being worth
#: seconds at all. A tool call is interesting at 0.1s resolution; an hour-long
#: chat is not, and "63m 04s" is a number you have to do arithmetic on to read.
DECIMAL_UNDER_SECONDS = 10
MINUTE, HOUR = 60, 3600
...
def duration(seconds: float | None) -> str:
    ...
    if seconds < DECIMAL_UNDER_SECONDS:
        return f"{seconds:.1f}s"
    if seconds < MINUTE:
        return f"{seconds:.0f}s"
    if seconds < HOUR:
        return f"{int(seconds // MINUTE)}m {int(seconds % MINUTE):02d}s"
    return f"{int(seconds // HOUR)}h {int((seconds % HOUR) // MINUTE):02d}m"


#: Decimal units, because that is what the vendor prints: `ollama list` says
#: 5.2 GB for the file the model picker is about to name, and a picker saying
#: 4.8 GiB for the same file is two numbers for one fact.
SIZE_UNITS = ("B", "kB", "MB", "GB", "TB")
SIZE_STEP = 1000


def size(n: int | None) -> str:
    """`274 MB` / `5.2 GB`. ``None`` reads as unknown, never zero.

    Bytes on disk or in memory: a model's weights, a source file. One decimal
    from megabytes up, because a picker choosing between 5.2 and 13 GB on a
    16 GB machine needs the decimal and a directory listing does not mind it.
    """
    if n is None:
        return "—"
    value = float(max(int(n), 0))
    unit = SIZE_UNITS[0]
    for unit in SIZE_UNITS:
        if value < SIZE_STEP or unit == SIZE_UNITS[-1]:
            break
        value /= SIZE_STEP
    if unit in SIZE_UNITS[:2]:
