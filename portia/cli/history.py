...
def render_summary(summary: dict) -> str:
    ...
    lines = [
        ...
        f" {summary['approved']} allowed, {summary['refused']} refused{_automatic(summary)}",
        f"  ended     {summary.get('subtype')}{_took(summary)}{_cost(summary)}",
    ]
    return "\n".join(lines)
...
def render_replay(event: events.Event) -> None:
    ...
    if event.kind == events.PROMPT:
        ...
        ran_on = f"{provider} · {model}" if provider else model
        print(f"\n{'─' * 60}\n> {event.data.get('text', '')}  [{ran_on}]")
        ...
        # `!` is *this stopped for you*. A write nobody was asked about did not,
        # and marking it the same way would make the terminal claim something
        # the window does not (`ui/transcript._resolved_write_view`).
        mark = "·" if event.data.get("auto") else "!"
        verb = "wrote" if event.data.get("auto") else "wants to write"
        print(f"\n  {mark} {events.tool_label(str(event.data.get('name')))} {verb}:")
        ...
        if event.data.get("auto"):
            print("     → not asked")
            return
        word = "allowed" if event.data.get("allowed") else "refused"
        waited = event.data.get("waited")
        # How long the loop sat here, when the log knows. It is not derivable
        # from the stamps either side (`agent/ask.py`), and it is a fact about
        # the wait rather than a remark about the human.
        sat = f" after {duration(float(waited))}" if waited is not None else ""
        print(f"     → {word}{sat}")
    else:
        render_live(event)
...
def _automatic(summary: dict) -> str:
    """How many of the allowed writes never stopped for anyone.

    Silent at zero, which is every log written before `ask.py` could be told to
    skip a tool, and every session that gated all of them. Said as a
    qualification of `allowed` rather than as its own line, because that is what
    it is: the count of writes that happened is right either way, and this says
    how many of them were decisions.
    """
    automatic = summary.get("auto_approved") or 0
    return f" ({automatic} not asked)" if automatic else ""


def _took(summary: dict) -> str:
    """Wall-clock, when the log stamped its records. Silent when it did not.

    A log written before `runlog.STAMP_FORMAT` existed has no times at all, and
    printing `0s` for it would state something false about a run that took
    minutes — the same reason `_total_tokens` keeps ``None`` as ``None``.
    """
    seconds = summary.get("wall_seconds")
    if seconds is None:
        return ""
    # What share of it was portia stopped, when the log knows. Said as part of
    # the same phrase because it is part of the same number, and a run that
    # spent most of its wall clock waiting for a human is describing the human's
    # reading, not portia's speed (`runlog.summary` → `waited_seconds`).
    waited = summary.get("waited_seconds")
    sat = f" ({duration(waited)} waiting on you)" if waited is not None else ""
    return f" in {duration(seconds)}{sat}"
