...
plus any step the model does not read, which is the form CI wants.
"""
...
def main() -> None:
    ...
    if args.check:
        ...
        # An unread step used to be drawn in the app as a fact, because it arrived
        # honestly: `record_step` was append-only and was the only way to ask the
        # data a question. Neither is true now, so it is a bug and this is where a
        # bug goes (`docs/FINDINGS.md` §7).
        unread = pipeline.unread_steps(args.root)
        for name, steps in unread.items():
            print(f"unread {name} — the table does not read: {', '.join(steps)}")
        if stale or unread:
            raise SystemExit(1)
        print("every generated model matches its spec, and reads every step it declares")
        return
