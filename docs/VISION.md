...
- **Only shows those** — even inside a big, complex repo, the left panel is a curated view of the
  data + our artifacts.
- *Refined 2026-08-02: the curation is a **filter over the real directory**, not a substitute for
  it.* The pane browses the project the way a file browser does, folders and all, and keeps a file
  if portia knows about it or if `core/io` registers a reader for its suffix. The version before
  this showed six flat lists and no paths, which meant a layered project's spec and its compiled
  model were two rows with the same name and no way to tell where either lived.
...
3. ~~**User can correct it.**~~ — *built 2026-07-27. The source inspector has two routes, and they
   ...
   correction survives a re-index either way. The project brief is editable from the row at the top
   of the left pane, and from Settings.*
...
`VISION.md` asks how we decide what to surface inside a big repo. The answer is **a file appears if
portia knows about it** — and since 2026-08-02 that is a *filter over a real directory walk* rather
than six flat lists.

A file is drawn if the catalog knows it, or `spec.discover_specs` does (which finds specs in layer
subdirectories and enforces the one-name-per-project rule), or it is a compiled model, a written
...
name, which is the first question anyone asks of a pipeline they have been handed. Six known folders
was also a shape the window was imposing on the agent, and the folders are not portia's to fix — a
tree adapts to whatever a project turns out to look like, and a fixed list of sections does not.

**Models are still the deliverable**, they are just `models/` now: a run's CSV under `out/` is a
result of executing the pipeline, `models/*.sql` *is* the pipeline. Layers are folders, in build
order, with no tier louder than another — which is what they always were on disk.

Clicking a source shows its catalog entry — the prose summary, the per-column roles, the check
facts. Clicking a file portia can read but has never profiled offers to index it, which is the
profiling half only: free, deterministic, no model turn.

Two things are pinned outside the tree, because `.portia/` is not walked: **the project brief** as a
row at the top, which opens an editor in the middle pane, and **Turns** as a section at the foot.
Showing the catalog's own YAML as files would invite hand-editing the thing the copilot maintains.
...
| `cat .portia/project.yaml` (the brief) | The **Project brief** row at the top of the tree, editable |
| `index <one file already in the repo>` | Clicking a file marked **not indexed** → **Index it** |
| `ls` — what is actually in this project | The tree itself, which is the point of it being one |
...
- ~~**Workflow chaining:** how downstream workflows reference upstream outputs~~ — *decided
  ...
  **Still open:** spec versioning, drift across the chain, and how a workflow's "maturity" is
  represented.
- ~~**Left-panel curation:** how we decide what counts as a "data file or artifact" to surface
