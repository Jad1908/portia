...
def test_a_partial_drop_is_named_because_that_is_the_report_working(table):
    """The 17% worth keeping: columns that were meant to come through and did not."""
    left = pd.DataFrame({"k": [1, 2], "kept": ["a", "b"]})
    right = pd.DataFrame({"k": [1, 2], "title": ["x", "y"], "rank": [1, 2]})
    out = table(pd.DataFrame({"k": [1, 2], "kept": ["a", "b"], "title": ["x", "y"]}), "out")
    report = outcome_report(
        out, inputs={"left": table(left, "l"), "right": table(right, "r")}, keys={}
    )

    assert report["contribution"]["right"]["columns_dropped"] == ["rank"]


def test_nothing_reaching_the_output_reports_no_drops_at_all(table):
    """`SELECT count(*)` did not *drop* 191 columns. It counted rows.

    `n_columns: 0` already says no input column reached the output. The list
    that used to sit beside it was the input's whole schema restated — 24,999 of
    `columns_dropped`'s 30,049 characters in the AQN build run, on seven steps
    that were each doing exactly what they were written to do.

    Absent, not `[]`: an empty list reads as *nothing was dropped*, which is the
    opposite of what happened. Absence is the same thing the encoder and the
    knowledge graph already mean by it — nobody measured this.
    """
    src = pd.DataFrame({"id": [1, 2], "name": ["a", "b"], "note": ["x", "y"]})
    counted = table(pd.DataFrame({"n": [2]}), "counted")
    report = outcome_report(counted, inputs={"src": table(src, "src")}, keys={})

    contribution = report["contribution"]["src"]
    assert contribution["n_columns"] == 0
    assert contribution["contributed"] is None
    assert "columns_dropped" not in contribution
    assert NO_CONTRIBUTION not in report["flags"], "declining to judge is not a finding"
    render_outcome(report)  # the renderer must not reach for the absent key


def test_a_step_that_renames_everything_is_why_dropped_was_the_wrong_word(table):
    """Matching is by name, so this carries every value through and still lands
    in the same state as an aggregate. Calling those columns dropped is a claim
    the measurement cannot make, which is the second reason the list is gone."""
    src = pd.DataFrame({"id": [1, 2], "name": ["a", "b"]})
    renamed = table(pd.DataFrame({"hotel_id": [1, 2], "hotel_name": ["a", "b"]}), "renamed")
    report = outcome_report(renamed, inputs={"src": table(src, "src")}, keys={})

    assert report["contribution"]["src"]["n_columns"] == 0
    assert "columns_dropped" not in report["contribution"]["src"]
...
def test_report_keys_still_names_every_field_a_report_comes_back_with(bookings, events, table):
    """`agent.handlers` reads this set to tell one confusion apart from a typo.

    A step's `expect` asserts the *op's* report; these are the words of the other
    report that arrives from the same call, and a run reaching for `n_rows` gets
    told which one it belongs to. If a field is added here and this set is not
    told, that message quietly degrades back to "never reports" — the wrong
    answer this constant exists to replace.
    """
    full = _report(table, bookings, events, how="left", keys=["hotel_id"], grain=["hotel_id"])
    empty = outcome_report(
        table(bookings.head(0), "rk1"), inputs={"b": table(bookings, "rk2")}, grain=["hotel_id"]
    )
    assert set(full) == set(REPORT_KEYS)
    assert set(empty) == set(REPORT_KEYS), "an empty table reports the same fields"
...
def test_a_grain_claim_is_matched_the_way_the_engine_spells_the_column(table):
    """The 2026-09-06 Snowflake sessions: `AS establishment_id` came back as
    ESTABLISHMENT_ID, the claim said `establishment_id`, and the step was refused
    as `grain_columns_missing` with the column right there. The claim is matched
    the way the engine folds it, and the report names the real column."""
    out = table(pd.DataFrame({"ESTABLISHMENT_ID": ["a", "b", "b"], "N": [1, 2, 3]}))
    report = outcome_report(out, inputs={}, keys={}, grain=["establishment_id"])
    grain = report["grain"]
    assert grain["measurable"] is True
    assert grain["keys"] == ["ESTABLISHMENT_ID"]
    assert grain["unique"] is False and GRAIN_NOT_UNIQUE in report["flags"]
