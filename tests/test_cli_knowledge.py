...
def test_cypher_shows_what_a_write_would_send_and_sends_nothing(project, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "CREATE CONSTRAINT source_project_key" in out
    assert "MERGE (a)-[r:DERIVES_FROM]->(b)" in out
    # Every statement names the project: one server holds them all, and the
    # prune deletes on absence (`knowledge/schema.py`'s `PROJECT`).
    assert "n.project = $project" in out
