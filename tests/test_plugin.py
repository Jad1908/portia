...
def test_the_skill_is_what_the_prompts_render_to():
    ...
    assert plugin.SKILL.read_text(encoding="utf-8") == plugin.render()


def test_the_method_is_in_the_skill_word_for_word():
    from portia.agent import prompts

    assert prompts.load("copilot") in plugin.SKILL.read_text(encoding="utf-8")


def test_nothing_is_left_unfilled():
    body = plugin.SKILL.read_text(encoding="utf-8")
