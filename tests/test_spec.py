...
def test_run_spec_names_each_step_before_it_runs(project):
    """A surface can only say which step is executing if it is told *before*.

    Reported after the fact the name would always be the step that just
    finished, which on the one step worth naming — the slow one — is precisely
    the name you do not have while you are waiting for it.
    """
    tmp_path, spec = project
    spec = {
        **spec,
        "steps": [
            spec["steps"][0],
            {
                ...
                "op": "sql",
                "inputs": ["joined"],
                "sql": "select customer_id, count(*) as n from joined group by customer_id",
            },
        ],
    }
    seen = []

    run_spec(spec, base_dir=tmp_path, on_step=lambda *args: seen.append(args))

    assert seen == [(0, 2, "joined"), (1, 2, "counted")]


def test_run_spec_without_a_callback_is_the_terminal(project):
    """`on_step` is optional and the CLI passes nothing — the default must not
    become a thing every caller has to know about."""
    tmp_path, spec = project
    assert len(run_spec(spec, base_dir=tmp_path)) == 1
