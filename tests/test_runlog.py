...
def test_a_chat_is_one_file_opened_by_a_header_then_the_prompts_it_read(tmp_path):
    ...
    assert json.loads(first)["data"]["text"] == "build me a table"
    written = json.loads(last)
    assert written["kind"] == "text"
    assert written["data"] == {"text": "on it"}
    assert written["at"], "every record is stamped with when it was written"


def test_the_header_line_stays_small_so_a_list_can_be_drawn_from_it(tmp_path):
    """`read_header` is one line per row of the app's left pane, redrawn per event.

    The prompts are ~30 KB — larger than a whole chat log — so they are the
    second record and not part of the header. Putting them on line one would
    have made the cheapest read in the module the most expensive one, in the
    ...
    assert runlog.read_header(log.path)["kind"] == runlog.CHAT
...
def test_each_event_is_on_disk_before_the_next_one_happens(tmp_path):
    ...
    _turn(log, _call("profile_source", name="reservations"))
...
def test_time_parked_on_a_human_is_summed_across_writes_and_questions(tmp_path):
    """`wall_seconds` cannot split its own number; this is the half it hides.

    Its own comment says four minutes profiling and four minutes waiting are
    the same number. They are, and this is what tells them apart.
    """
    log = _chat(tmp_path, prompt="p", model="m")
    _turn(
        log,
        events.approval_result_event("mcp__portia__record_step", True, 200.3),
        events.approval_result_event("mcp__portia__record_step", False, 82.1),
        events.answer_event({"which grain?": "city-date"}, 317.3),
    )

    assert runlog.summary(runlog.read(log.path))["waited_seconds"] == 599.7


def test_a_log_that_never_measured_a_wait_says_nothing_rather_than_zero(tmp_path):
    """Every log written before `agent/ask.py` timed its own awaits.

    Zero would claim the human answered instantly — the same lie `wall_seconds`
    refuses to tell about a run written before records were stamped.
    """
    log = _chat(tmp_path, prompt="p", model="m")
    _turn(log, events.approval_result_event("mcp__portia__record_step", True))

    assert runlog.summary(runlog.read(log.path))["waited_seconds"] is None


def test_a_wait_of_zero_is_still_a_measurement(tmp_path):
    """Answered before the renderer drew it. Measured, and not the same as absent."""
    log = _chat(tmp_path, prompt="p", model="m")
    _turn(log, events.approval_result_event("mcp__portia__record_step", True, 0.0))

    assert runlog.summary(runlog.read(log.path))["waited_seconds"] == 0.0
...
# --- when it happened, and what it was reading -------------------------------


def test_every_event_carries_when_it_was_written(tmp_path):
    log = _chat(tmp_path, events.Event(events.TEXT, {"text": "on it"}), cwd=tmp_path)
    run = runlog.read(log.path)

    assert len(run.times) == len(run.events)
    assert all(at for at in run.times)
    assert [e.kind for e, _ in run.timed()] == [e.kind for e in run.events]


def test_a_log_written_before_stamps_reads_as_unknown_not_as_instant(tmp_path):
    """Read, never migrated — and a missing duration must not render as zero."""
    legacy = tmp_path / runlog.DIR_FOR_KIND[runlog.CHAT]
    legacy.mkdir(parents=True)
    path = legacy / "2026-07-29T16-32-57.jsonl"
    path.write_text(
        '{"kind": "header", "data": {"kind": "chat"}}\n'
        '{"kind": "text", "data": {"text": "no stamp on this one"}}\n',
        encoding="utf-8",
    )
    run = runlog.read(path)

    assert run.times == [None]
    assert run.timed() == [(run.events[0], None)]
    assert runlog.summary(run)["wall_seconds"] is None


def test_elapsed_refuses_to_invent_a_number(tmp_path):
    assert runlog.elapsed("2026-08-09T10:00:00.000", "2026-08-09T10:00:02.500") == 2.5
    assert runlog.elapsed(None, "2026-08-09T10:00:02.500") is None
    assert runlog.elapsed("2026-08-09T10:00:00.000", None) is None
    assert runlog.elapsed("not a time", "2026-08-09T10:00:02.500") is None


def test_wall_time_is_measured_from_the_records_not_the_header(tmp_path):
    log = runlog.start(tmp_path, cwd=tmp_path)
...
def test_the_prompts_the_run_read_are_kept_beside_it(tmp_path):
    """`portia_sha` finds them; it does not let you read them without leaving
    what you are doing — and the L1 brief is not in git at any sha."""
    log = _chat(tmp_path, cwd=tmp_path)
    run = runlog.read(log.path)

    assert run.prompts["system"], "L0 + the project brief, composed"
    assert "record_step" in run.prompts["tools"]
    # The *rendered* description, not the template: `record_step.md` has
    # `{expect_sql}` where the model has the actual field list.
    assert "{expect_sql}" not in run.prompts["tools"]["record_step"]
    assert "result_rows" in run.prompts["tools"]["record_step"]


def test_a_log_that_could_not_compose_the_prompts_is_still_written(monkeypatch, tmp_path):
    """Best-effort, like the graph writes: a record *about* the run must never
    stop the run (`CLAUDE.md` → knowledge/)."""
    monkeypatch.setattr(runlog, "prompts_read", lambda *_a, **_k: {})
    log = _chat(tmp_path, events.Event(events.TEXT, {"text": "on it"}), cwd=tmp_path)
    run = runlog.read(log.path)

    assert run.prompts == {}
    assert [e.kind for e in run.events] == [events.PROMPT, events.TEXT]
...
def test_one_file_holds_every_exchange(tmp_path):
    ...
    _turn(log, _call("join_findings", left="reservations"), _result())
    ...
    assert facts["sequence"] == ["join_findings(reservations)", "record_step"]
...
def test_writes_nobody_was_asked_about_are_counted_apart(tmp_path):
    """`approved` counts the writes that happened, not the decisions somebody
    made. Without the split, a session that auto-allowed 26 catalog writes would
    report 26 approvals nobody gave — and "writes refused" is one of the few
    things this log can measure without an answer key."""
    log = _chat(tmp_path, prompt="p", model="m")
    _turn(
        log,
        events.approval_result_event("mcp__portia__set_interpretation", True, auto=True),
        events.approval_result_event("mcp__portia__set_interpretation", True, auto=True),
        events.approval_result_event("mcp__portia__record_step", True, 82.1),
        events.approval_result_event("mcp__portia__record_step", False, 12.0),
    )

    summary = runlog.summary(runlog.read(log.path))
    assert (summary["writes"], summary["approved"], summary["refused"]) == (4, 3, 1)
    assert summary["auto_approved"] == 2
    # One human approval, and it is the one that took a minute of reading.
    assert summary["approved"] - summary["auto_approved"] == 1
    assert summary["waited_seconds"] == 94.1


def test_a_log_with_no_automatic_writes_says_zero(tmp_path):
    """Zero here is a real count, not an absence: every write stopped for
    someone. It is what every log written before this existed also reports, and
    that reads correctly for them too."""
    log = _chat(tmp_path, prompt="p", model="m")
    _turn(log, events.approval_result_event("mcp__portia__record_step", True))

    assert runlog.summary(runlog.read(log.path))["auto_approved"] == 0


def test_call_durations_come_off_the_stamps_by_call_id(tmp_path):
    from portia.agent.events import Event

    log = runlog.start(tmp_path, cwd=str(tmp_path), kind=runlog.CHAT)
    log.write("tool_call", {"id": "a", "name": "x", "input": {}})
    log.write("tool_call", {"id": "b", "name": "y", "input": {}})
    log.write("tool_result", {"id": "a", "text": "…", "is_error": False})
    transcript = runlog.read(log.path)
    took = runlog.call_durations(transcript)
    assert set(took) == {"a"}  # `b` never came back
    assert took["a"] >= 0.0
    # A log with no stamps at all has nothing to say, and says nothing.
    bare = runlog.Transcript(
        path=log.path, events=[Event("tool_call", {"id": "a"}), Event("tool_result", {"id": "a"})]
    )
    assert runlog.call_durations(bare) == {}


# --- picking a chat up (`docs/CHAT_SESSIONS.md` §3.3, §3.9) --------------------


def test_a_resumed_chat_appends_to_its_own_file(tmp_path):
    """The file is the chat (§5): a later process goes on in it, after a mark
    saying so and the prompts *that* process's model read."""
    from portia.agent import events

    log = runlog.start(tmp_path, kind=runlog.CHAT)
    log.event(events.prompt_event("first", model="m", effort="low"))
    log.event(events.Event(events.RESULT, {"session_id": "abc", "subtype": "success"}))

    again = runlog.resume(log.path, tmp_path)
    assert again.path == log.path
    again.event(events.prompt_event("second", model="m", effort="low"))

    run = runlog.read(log.path)
    kinds = [e.kind for e in run.events]
    assert kinds == [events.PROMPT, events.RESULT, events.PROMPT]
    assert run.resumes == [2], "the first event written after the resume"
    assert runlog.summary(run)["resumes"] == 1
    assert runlog.summary(run)["exchanges"] == 2
    assert runlog.summary(run)["session_id"] == "abc"
    assert runlog.logs_in(tmp_path) == [log.path], "no second file"


def test_resuming_a_log_that_is_not_there_is_refused(tmp_path):
    with pytest.raises(FileNotFoundError):
        runlog.resume(tmp_path / "chats" / "nope.jsonl", tmp_path)


def test_a_listing_reads_the_top_of_the_file_and_stops(tmp_path):
    """One row per chat, off the header and the first prompt, never a parse of
    the whole log — the counts belong to the chat once it is open."""
    from portia.agent import events

    log = runlog.start(tmp_path, kind=runlog.CHAT)
    log.event(
        events.prompt_event("merge the two customer files\nthen dedupe", model="m", effort="low")
    )
    log.event(events.prompt_event("a follow-up nobody lists by", model="m2", effort="high"))

    row = runlog.read_listing(log.path, tmp_path)
    assert row["kind"] == runlog.CHAT
    assert row["title"] == "merge the two customer files", "the first line of the first prompt"
    assert row["prompt"].startswith("merge the two")
    assert row["model"] == "m" and row["effort"] == "low"
    assert row["legacy"] is False
    assert row["started"] == runlog.read_header(log.path)["started"]


def test_a_legacy_log_lists_as_legacy(tmp_path):
    """Written before the rename, under `.portia/runs/`: readable, and marked so
    a surface can say why it cannot be continued."""
    legacy = tmp_path / runlog.LEGACY_DIR
    legacy.mkdir()
...
def test_a_title_lives_in_a_sidecar_and_never_in_the_log(tmp_path):
    from portia.agent import events

    log = runlog.start(tmp_path, kind=runlog.CHAT)
    log.event(events.prompt_event("first prompt", model="m", effort=None))
    size = log.path.stat().st_size

    runlog.set_title(log.path, "customers, deduped", tmp_path)
    assert log.path.stat().st_size == size, "the log was not touched"
    assert runlog.read_listing(log.path, tmp_path)["title"] == "customers, deduped"
    assert runlog.titles(tmp_path)[runlog.title_key(log.path)] == "customers, deduped"

    runlog.set_title(log.path, "   ", tmp_path)
    assert runlog.read_listing(log.path, tmp_path)["title"] == "first prompt"
    assert runlog.titles(tmp_path)[runlog.title_key(log.path)] is None


def test_deleting_a_log_takes_its_title_with_it(tmp_path):
    log = runlog.start(tmp_path, kind=runlog.CHAT)
    runlog.set_title(log.path, "gone soon", tmp_path)

    runlog.delete(log.path, tmp_path)
