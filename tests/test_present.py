...
def test_a_duration_reads_at_the_scale_it_is():
    assert duration(0.84) == "0.8s"
    assert duration(12.4) == "12s"
    assert duration(187) == "3m 07s"
    assert duration(3860) == "1h 04m"


def test_an_unknown_duration_is_not_zero():
    """A log written before portia stamped its records has no times at all, and
    `0.0s` would state something false about a run that took minutes."""
