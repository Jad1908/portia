...
# --- identifier case, which was silently fatal on every real extract ---------


def test_upper_case_columns_are_traced_and_reported_as_the_catalog_spells_them():
    """The 2026-09-02 bug, and the reason it survived its tests.

    `qualify` folds an unquoted identifier to lower case, because that is what
    DuckDB does resolving one. `_origins` then looked for ``id`` in a catalog
    holding ``ID`` and matched nothing — so **every** column of **every** model
    built from an upper-case source lost its lineage, and the app drew "columns
    not known — index this source" about sources that were fully indexed.

    Every fixture in this repo is lower case and every real extract is not: on
    the AQN project all three specs traced zero of their columns, 0 DERIVES_FROM
    edges against 37 after the fix.
    """
    got = column_origins("SELECT ID, TITLE AS T FROM ev", {"ev": ["ID", "TITLE"]})

    assert got == {
        "ID": [Origin("ev", "ID", False)],
        "T": [Origin("ev", "TITLE", False)],
    }


def test_the_output_names_are_what_the_sql_wrote_not_what_qualify_folded():
    """DuckDB preserves the case you type, so a model's Column nodes have to too."""
    got = column_origins("SELECT ID FROM ev", {"ev": ["ID"]})
    assert list(got) == ["ID"]


def test_a_star_takes_its_spelling_from_the_declared_inputs():
    """The one case the pre-qualify select list cannot supply, because it is a star."""
    got = column_origins("SELECT * FROM ev", {"ev": ["ID", "TITLE"]})
    assert list(got) == ["ID", "TITLE"]


def test_a_quoted_identifier_loses_its_quotes_before_it_is_matched():
    """A leaf off `SELECT "ID"` arrives as `ev."id"` — folded *and* quoted.

    DuckDB is case-insensitive even for quoted identifiers, so sqlglot normalizes
    both. The quotes are part of the rendering and never part of the name.
    """
    got = column_origins('SELECT "ID" AS "Id" FROM ev', {"ev": ["ID"]})
    assert got == {"Id": [Origin("ev", "ID", False)]}
