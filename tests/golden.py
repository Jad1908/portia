...
JOIN_PAIRS: dict[str, dict] = {
    ...
    "reservations_hotels": {"left": "reservations", "right": "hotels", "on": ["hotel_id"]},
    "hotels_events": {
        "left": "hotels",
        "right": "city_events",
        "left_on": ["city"],
...
CASES: tuple[Case, ...] = (
    ...
    *(Case(f"join_findings/{n}", "join_findings", p) for n, p in JOIN_PAIRS.items()),
    ...
    _spec(
        "hotels_chain_fanout",
        {"reservations": "reservations", "hotels": "hotels", "city_events": "city_events"},
        [
            _CLEAN_EVENTS,
            _CLEAN_HOTELS,
            _BOOKINGS,
        ...
        [
            {
                ...
                "op": "sql",
                "inputs": ["reservations"],
                "sql": "SELECT * FROM reservations WHERE hotel_id = 'H000'",
            }
        ],
    ),
    ...
    _spec(
        "no_contribution",
        {"reservations": "reservations", "city_events": "city_events"},
        [
            {
                "id": "bookings_events",
                "op": "join",
    ...
    _spec(
        ...
        {"reservations": "reservations", "hotels": "hotels"},
        [
            {
                "id": "bookings",
                "op": "join",
