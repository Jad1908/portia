...
def test_to_numeric_reports_failures_not_silently(table):
    df = pd.DataFrame({"amt": ["1", " 2 ", "N/A", "4"]})
    res = apply_normalize(table(df), [{"column": "amt", "op": "to_numeric"}])
    assert ptypes.is_numeric_dtype(res.table.head(100)["amt"])
    assert list(res.table.head(100)["amt"].dropna()) == [1.0, 2.0, 4.0]  # whitespace parsed too
    rec = _rec(res, "amt")
...
def test_to_numeric_with_fill(table):
    df = pd.DataFrame({"amt": ["1", "N/A", "3"]})
    res = apply_normalize(table(df), [{"column": "amt", "op": "to_numeric", "fill": 0}])
    assert list(res.table.head(100)["amt"]) == [1.0, 0.0, 3.0]


def test_strip_counts_changes(table):
    df = pd.DataFrame({"c": ["a ", " b", "c", "d "]})
    res = apply_normalize(table(df), [{"column": "c", "op": "strip"}])
    assert list(res.table.head(100)["c"]) == ["a", "b", "c", "d"]
    assert _rec(res, "c")["n_changed"] == 3


def test_lower_counts_changes(table):
    df = pd.DataFrame({"c": ["A", "Bc", "d"]})
    res = apply_normalize(table(df), [{"column": "c", "op": "lower"}])
    assert list(res.table.head(100)["c"]) == ["a", "bc", "d"]
    assert _rec(res, "c")["n_changed"] == 2


def test_to_string_changes_dtype(table):
    df = pd.DataFrame({"k": [1, 2, 3]})
    res = apply_normalize(table(df), [{"column": "k", "op": "to_string"}])
    assert ptypes.is_string_dtype(res.table.head(100)["k"])
    assert list(res.table.head(100)["k"]) == ["1", "2", "3"]


def test_does_not_mutate_input(table):
    df = pd.DataFrame({"c": ["a ", "b "]})
    apply_normalize(table(df), [{"column": "c", "op": "strip"}])
    assert list(df["c"]) == ["a ", "b "]  # original untouched


def test_unknown_op_and_missing_column_raise(table):
    df = pd.DataFrame({"c": ["a"]})
    with pytest.raises(ValueError, match="unknown transform op"):
        apply_normalize(table(df), [{"column": "c", "op": "titlecase"}])
    with pytest.raises(ValueError, match="no such column"):
        apply_normalize(table(df), [{"column": "nope", "op": "strip"}])


def test_on_messy_fixture_flags_the_bad_values(table):
    # messy_customers.signup_amount is numeric-stored-as-text with 'N/A'/'pending'.
    res = apply_normalize(
        table(messy_customers()), [{"column": "signup_amount", "op": "to_numeric"}]
    )
...
def test_provenance_keys_declaration_matches_reality(table):
    ...
    result = apply_normalize(table(messy_customers()), [{"column": "signup_amount", "op": "strip"}])
    assert set(result.provenance) == set(PROVENANCE_KEYS)


def test_transform_ops_declaration_matches_the_dispatch(table):
    ...
    for op in TRANSFORM_OPS:  # every declared op is accepted
        apply_normalize(table(df), [{"column": "signup_amount", "op": op}])
    with _pytest.raises(ValueError, match="unknown transform op"):
        apply_normalize(table(df), [{"column": "signup_amount", "op": "nope"}])


def test_a_column_named_in_another_case_is_the_engine_s_column(table):
    df = pd.DataFrame({"AMT": [" 1 ", "2"]})
    res = apply_normalize(table(df), [{"column": "amt", "op": "to_numeric"}])
    assert _rec(res, "AMT")["n_converted"] == 2
    assert list(res.table.columns) == ["AMT"]
